...
//
// ===================================================================================
// == VERSION 9.5 REFACTOR: WORKER COMPLETION LOGIC                                 ==
// ===================================================================================
// - Replaced the timeout-based "straggler" system with a new "late to dinner" model.
// - The system now dispatches a fixed number of extra jobs (5) beyond the required
//   population size.
// - A generation is now considered complete as soon as the required number of unique
//   chromosome evaluations are finished.
// - Any results from the extra workers that arrive after this point ("late to dinner")
//   are discarded, preventing slow simulations from delaying the entire evolution.
// - Renamed "straggler" in the breeding pool to "underdog" to avoid confusion.
// ===================================================================================
...
const BREEDING_POOL_RATIOS = {
    best: 0.95,
    underdog: 0.025, // Renamed from straggler
    newcomer: 0.025
};
...
const SOLUTION_SPACE_CONFIG = {
    ...
    ramps: [
        { y_pos: { min: 150, max: 300 }, angle: { min: 0.09, max: 0.2 } },
        { y_pos: { min: 425, max: 650 }, angle: { min: 0.09, max: 0.2 } },
        { y_pos: { min: 750, max: 950 }, angle: { min: 0.25, max: 0.55 } },
        { y_pos: { min: 1250, max: 1550 }, angle: { min: 0.3, max: 0.6 } },
        { y_pos: { min: 1800, max: 2300 } }
    ]
};
...
class EvolutionEngine {
    constructor() {
        ...
        this.lateToDinnerWorkerCount = 5; // New property for extra workers
    ...
    async runGeneration() {
        ...
        if (individualsToEvaluate.length > 0) {
            ...
            allJobs.sort(() => Math.random() - 0.5); // Shuffle to mix in late jobs
    ...
    processWorkerResult(e) {
        if (!this.jobQueue || this.jobQueue.isResolved) {
            return; // Generation is already complete, ignore late results.
        }

        const { result, id } = e.data.payload;
        const individual = this.population.find(p => p && p.id === id);

        // Only process the result if the individual hasn't already received one.
        // This handles cases where a duplicate "late_to_dinner" job finishes before the original.
    ...
    // ========================================================================
    // == PINCH POINT VALIDATION LOGIC (START)                               ==
    // ========================================================================
    ...
    // ========================================================================
    // == PINCH POINT VALIDATION LOGIC (END)                                 ==
    // ========================================================================
    ...
    createRandomChromosome() {
        ...
        const lastRampY = c.cascadingRamps[c.cascadingRamps.length - 1].y_position;
        const maxDetectorHeight = lastRampY - 100;
        c.detectorHeight = randInt(cfg.detectorHeight.min, maxDetectorHeight);
    ...
    mutate(chromosome) {
        ...
        if (Math.random() < rate) chromosome.detectorHeight = clamp(Math.round(chromosome.detectorHeight + randn_bm(-10, 10)), cfg.detectorHeight.min, cfg.detectorHeight.max);
        ...
        chromosome.cascadingRamps.forEach((ramp, i) => {
            ...
            if (cfg.ramps[i].angle && Math.random() < rate) {
                const mutatedAngle = ramp.angle + randn_bm(-0.05, 0.05);
                ramp.angle = clamp(Math.abs(mutatedAngle), cfg.ramps[i].angle.min, cfg.ramps[i].angle.max);
            }
        });

        const lastRampY = chromosome.cascadingRamps[chromosome.cascadingRamps.length - 1].y_position;
        const maxDetectorHeight = lastRampY - 100;
        chromosome.detectorHeight = clamp(chromosome.detectorHeight, cfg.detectorHeight.min, maxDetectorHeight);
